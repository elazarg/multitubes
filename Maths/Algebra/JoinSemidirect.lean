...
* `Maths.SupBotMulAction`: a monoid action on a join-semilattice with bottom
  that preserves binary joins and the bottom element.
* `Maths.JoinSemidirectTransport.Label`: a floor together with an acting monoid
  element, with `Maths.JoinSemidirectTransport.Label.apply` for its action and
  `Maths.JoinSemidirectTransport.Label.comp` for chronological composition.
* `Maths.JoinSemidirectTransport.Label.instMonoid`: the semidirect-product
  monoid structure on labels.

## Main results

* `Maths.JoinSemidirectTransport.Label.apply_comp`: composition of labels
  agrees with composition of their actions.
* `Maths.JoinSemidirectTransport.Label.instMulAction`: labels act on the
