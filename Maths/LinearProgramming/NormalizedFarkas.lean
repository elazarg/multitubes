...
`Maths.LinearProgramming.StandardForm`: a bounded slice of the certificate cone that
...
comes from `Maths.LinearProgramming.StandardForm`, which this file re-exports.

## Main definitions

* `Maths.LinearProgramming.normalizedFarkasMatrix`: a homogeneous balance matrix with
  a mass row appended.
* `Maths.LinearProgramming.normalizedFarkasRhs`: the matching right-hand side, zero on
  the balance rows and one on the mass row.
* `Maths.LinearProgramming.normalizedFarkasCertificateSet`: the set of nonnegative
  vectors that balance and have unit mass, as a standard-form fiber.

## Main results

* `Maths.LinearProgramming.mem_normalizedFarkasCertificateSet`: membership, unpacked
  into the balance and unit-mass conditions.
* `Maths.LinearProgramming.isClosed_normalizedFarkasCertificateSet`: the certificate
  set is closed.
-/
