...
import Maths.Multitubes.Additive.CycleMean
...
modulus `Maths.Multitubes.MaxAffine.Contraction` pins one down uniquely, for every real
eigenvalue, and at nonnegative slopes straddling one
`Maths.Multitubes.MaxAffine.Eigenproblem` produces one by Brouwer's theorem in exponential
...
eigenvalues is in `Maths.Multitubes.MaxAffine.Spectrum`.
...
`Maths.Multitubes.MaxAffine.Farkas` and `Maths.Multitubes.MaxAffine.Slopes` provide.
