...
`Maths.Multitubes.MaxAffine.FixedPoint` solves the eigenvalue equation `F x = lam + x` for
...
(`Maths.MaxAffineTransport.Label.abs_apply_sub_apply_le`).  Taking a supremum over
...
subsume `Maths.MaxAffineTransport.exists_const_isLaxSection_of_slope_lt_one`, which
...
`Maths.MaxAffineTransport.exists_isEigenvector_of_slope_nonneg`, proved in
`Maths.Multitubes.MaxAffine.Eigenproblem` by Brouwer's theorem in exponential coordinates,
...
eigenvector are both non-unique, by `Maths.Multitubes.MaxAffine.Spectrum`, where the residual
question of the least eigenvalue is recorded.

## Main definitions

* `Maths.MaxAffineTransport.flipGraph`,
  `Maths.MaxAffineTransport.flipLabel`: two vertices, two reflecting edges, and no
  eigenvector.

## Main results

* `Maths.MaxAffineTransport.Label.abs_apply_sub_apply_le`: a max-affine map is
  Lipschitz with constant the absolute value of its slope.
* `Maths.MaxAffineTransport.abs_vertexOperator_sub_le`: the vertex operator does not
  increase a uniform edgewise bound.
* `Maths.MaxAffineTransport.lipschitzWith_vertexOperator`: the vertex operator is
  Lipschitz in the sup-norm with the largest absolute slope as constant.
* `Maths.MaxAffineTransport.contractingWith_vertexOperator`: **below unit modulus
  the vertex operator is a contraction.**
* `Maths.MaxAffineTransport.existsUnique_isEigenvector_of_abs_slope_lt_one`: **below
  unit modulus every real number is an eigenvalue, with exactly one eigenvector.**
* `Maths.MaxAffineTransport.existsUnique_fixedPoint_of_abs_slope_lt_one`: the case
  `lam = 0`, a unique genuine section.
* `Maths.MaxAffineTransport.vertexOperator_add_const_of_slope_eq`: homogeneity of
  degree the common slope, for a floorless labelling of one slope.
* `Maths.MaxAffineTransport.isEigenvector_add_const_of_slope_eq`: **outside unit
  slope the eigenvalue is not an invariant**, translating an eigenvector moves it.
* `Maths.MaxAffineTransport.not_exists_isEigenvector_flipLabel` and
  `Maths.MaxAffineTransport.not_forall_exists_isEigenvector`: **the eigenvalue
  equation can fail on a finite strongly connected graph once a slope is negative.**

## Implementation notes

The Lipschitz constant is taken in `ℝ≥0`, as `Maths.MaxAffineTransport` has no
...
junk value `0` of `Maths.MaxAffineTransport.vertexOperator` is constant in the
...
`Maths.MaxAffineTransport.isLaxSection_iff_vertexOperator_le`. -/
...
`Maths.MaxAffineTransport.vertexOperator_add_const`. -/
...
/-- The two reflecting labels of `Maths.MaxAffineTransport.flipGraph`: both have
...
/-- Each vertex of `Maths.MaxAffineTransport.flipGraph` has exactly one incoming
...
/-- Every vertex of `Maths.MaxAffineTransport.flipGraph` has an incoming edge. -/
theorem exists_target_flipGraph (vertex : Bool) : ∃ e : Bool, flipGraph.target e = vertex :=
  ⟨!vertex, by cases vertex <;> rfl⟩

/-- `Maths.MaxAffineTransport.flipGraph` is strongly connected: one edge joins the
...
`Maths.MaxAffineTransport.exists_isEigenvector_of_translation`, so what that theorem
