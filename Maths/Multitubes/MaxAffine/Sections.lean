...
`Maths.MaxAffineTransport` develops the obstruction side of max-affine
transport: at nonnegative slopes, a lax section forces the composite action of
every closed walk to have a pre-fixed point
(`Maths.MaxAffineTransport.holonomyApply_cycle_le`; without the slope
...
`Maths.Multitubes.MaxAffine.Farkas` settles them by a linear test, not a cyclewise
...
settled in `Maths.Multitubes.MaxAffine.FixedPoint`.  Mixed slopes below unit
modulus are settled metrically in `Maths.Multitubes.MaxAffine.Contraction`,
...
settled topologically in `Maths.Multitubes.MaxAffine.Eigenproblem`: an
...
* `Maths.MaxAffineTransport.Label.apply_const_le` -- the edgewise subunit-slope
  estimate: below unit slope, a constant above the floor and above the affine
  fixed point is a pre-fixed point.
* `Maths.MaxAffineTransport.exists_const_isLaxSection_of_slope_lt_one` -- with
  finitely many edges and all slopes below one, a constant lax section exists.
* `Maths.MaxAffineTransport.exists_isLaxSection_iff_forall_cycle_shift_nonpos` --
...
* `Maths.MaxAffineTransport.exists_isLaxSection_iff_forall_cycle_exists_prefixed`
  ...
  matching `Maths.MaxAffineTransport.Label.exists_apply_le_self_iff`.
...
the first branch of `Maths.MaxAffineTransport.Label.exists_apply_le_self_iff`,
...
`Maths.MaxAffineTransport.Label.exists_apply_le_self_iff`; the floors, however
...
theorem isPotential_of_isLaxSection {label : E → Label} (hslope : ∀ e : E, (label e).slope = 1)
    ...
    Maths.MaxPlusPotential.IsPotential G (fun e => (label e).shift) φ := fun e =>
...
theorem isLaxSection_of_isPotential {label : E → Label} (hslope : ∀ e : E, (label e).slope = 1)
    {φ : V → ℝ} (hφ : Maths.MaxPlusPotential.IsPotential G (fun e => (label e).shift) φ)
...
`Maths.MaxPlusPotential.exists_isPotential_iff_forall_closedWalk_nonpos` to
...
theorem exists_isLaxSection_iff_forall_cycle_shift_nonpos [Finite E]
    ...
    (∃ φ : V → ℝ, IsLaxSection G label φ) ↔
      ∀ (base : V) (cycle : G.Walk base base),
        Maths.MaxPlusPotential.walkWeight (fun e => (label e).shift) cycle ≤ 0 := by
  constructor
...
theorem shift_compList_edges {label : E → Label} (hslope : ∀ e : E, (label e).slope = 1)
    ...
    (Label.compList (walk.edges.map label)).shift
      = Maths.MaxPlusPotential.walkWeight (fun e => (label e).shift) walk := by
  rw [Label.shift_compList (fun f hf => ?_)]
  · rw [Maths.MaxPlusPotential.walkWeight, List.map_map]
    rfl
...
`Maths.MaxAffineTransport.Label.exists_apply_le_self_iff` decides from the
coefficients.  Together with
`Maths.MaxAffineTransport.holonomyApply_cycle_le` this closes the unit-slope case
of the duality: the necessary condition of weak duality is sufficient.  The
composite is the one that acts by transport,
`Maths.MaxAffineTransport.apply_compList_edges`. -/
