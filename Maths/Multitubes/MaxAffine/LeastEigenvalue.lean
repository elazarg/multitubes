...
`gamma_b + alpha_b * x (sigma_b) - x (tau_b) ≤ lam` of `Maths.Multitubes.MaxAffine.Duality`,
...
The branch system is the one of `Maths.Multitubes.MaxAffine.Duality` not a fresh
