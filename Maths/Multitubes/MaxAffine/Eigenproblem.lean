...
`Maths.Multitubes.MaxAffine.Spectrum` on one vertex carrying a reset of slope `0` and a
...
`Maths.Multitubes.MaxAffine.LeastEigenvalue` a least such level is the least eigenvalue
