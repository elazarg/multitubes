...
`Maths.Multitubes.Additive.Potentials` produces *sub*eigenvectors: functions with
...
Following `Maths.Multitubes.Additive.CycleMean`, the general-field theorem takes the
...
vertices, are `Maths.Multitubes.Additive.CriticalGraph`.  So are the critical *classes* - the
