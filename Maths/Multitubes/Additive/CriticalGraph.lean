...
`Maths.Multitubes.Additive.Eigenvector` produces one max-plus eigenvector for the maximum
...
`Maths.Multitubes.Additive.Condensation`; both are one-line reindexings of an ambient graph
