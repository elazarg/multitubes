...
`Maths.Multitubes.Additive.Potentials` characterizes the existence of a potential, that is
of a max-plus subeigenvector, by the absence of a closed walk of excessive mean weight, and
`Maths.Multitubes.Additive.ShortCycles` shows that the maximum cycle mean is attained on a
...
`Maths.Multitubes.Additive.ShortCycles` is stated over the reals.
