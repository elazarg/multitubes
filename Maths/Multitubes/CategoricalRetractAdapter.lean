...
/-!
# Categorical retract adapter for transport
...
retract theories: one-base flatness of a transport (`Maths.Multitubes.Exact`)
...
one, so the path-functor translation of `Maths.Multitubes.Category` carries an
...
* `Maths.Transport.compressed_walkMap_eq_via_category`: the concrete retract
  normal form of `Maths.Multitubes.Exact`, rederived through the path-functor dictionary.
-/
...
`Maths.Multitubes.Exact` proves the retract normal form concretely, by walk
induction, and `Maths.Multitubes.CategoricalRetracts` proves it categorically.
