...
`exp (F x v) = Φ (exp ∘ x) v`
(`Maths.MaxAffineTransport.exp_vertexOperator`).
...
`Maths.MaxAffineTransport.not_exists_isEigenvector_flipLabel`, where an eigenvector
provably fails to exist on a strongly connected graph.  The hypothesis `0 ≤ slope` is therefore
sharp.

## Main definitions

* `Maths.MaxAffineTransport.expWithBot`: the exponential of an element of `WithBot ℝ`,
  with `exp ⊥ = 0`.
* `Maths.MaxAffineTransport.coneTerm`: the exponential conjugate of a single label's
  demand, a monomial capped below by the exponential of the floor.
* `Maths.MaxAffineTransport.coneOperator`: the exponential conjugate of the vertex
  operator, defined on the closed cone.

## Main results

* `Maths.MaxAffineTransport.exp_apply_eq_coneTerm`: the edgewise conjugation identity.
* `Maths.MaxAffineTransport.exp_vertexOperator`: **the cone operator is the exponential
  conjugate of the vertex operator** on the open cone.
* `Maths.MaxAffineTransport.continuous_coneOperator`: the cone operator is continuous
  on all of `V → ℝ`, floors and slopes above one notwithstanding.
* `Maths.MaxAffineTransport.exists_source_eq`: on a strongly connected graph in which
  every vertex has an incoming edge, every vertex has an outgoing edge.
* `Maths.MaxAffineTransport.exists_mem_stdSimplex_coneOperator_eq`: **Brouwer's
  theorem applied to the normalized cone operator**, an eigenvector of `Φ` in the closed simplex.
* `Maths.MaxAffineTransport.exists_isEigenvector_of_slope_nonneg_of_incoming` and
  `Maths.MaxAffineTransport.exists_isEigenvector_of_slope_nonneg`: **a finite strongly
  connected graph whose slopes are all nonnegative carries an eigenvector of its vertex
  operator.**

## Implementation notes

`Maths.MaxAffineTransport.coneOperator` is defined through `WithBot ℝ` and carries the
junk value `0` at a vertex with no incoming edge, matching
`Maths.MaxAffineTransport.vertexOperator`; for nonnegative quantities `0` is also the
...
`Maths.MaxAffineTransport.exists_isEigenvector_of_slope_nonneg_of_incoming`.
...
`Maths.DirectedTransport.MaxAffine.Spectrum` on one vertex carrying a reset of slope `0` and a
doubling of slope `2`: its eigenvalues are exactly the reals from `5` upwards
...
`Maths.DirectedTransport.MaxAffine.LeastEigenvalue` a least such level is the least eigenvalue
whenever every vertex has an incoming edge, so the value is the optimum of an explicit finite linear
...
`Maths.MaxAffineTransport.not_forall_mem_relaxationLevels_iff_unitSlopeCycles_le`. What the Brouwer
argument supplies beyond that is the eigenvalues above the least one, which the linear program does
not describe.
...
On the open cone this is `exp` of `Maths.MaxAffineTransport.Label.apply` at the
logarithm, by `Maths.MaxAffineTransport.exp_apply_eq_coneTerm`. -/
...
`Maths.MaxAffineTransport.vertexOperator`, and unlike that operator it is defined and
...
faithful; `Maths.MaxAffineTransport.exists_isEigenvector_of_slope_nonneg` removes that
hypothesis. -/
...
`Maths.MaxAffineTransport.not_exists_isEigenvector_flipLabel`, where slope `-1` on a
...
`Maths.MaxAffineTransport.eigenvalues_loopLabel` and
`Maths.MaxAffineTransport.exists_pair_ne_isEigenvector_loopLabel`. -/
