...
import Maths.DirectedTransport.FiniteInequality.Sparse
...
* `Maths.MaxAffineTransport.RationalLabel` and
  `Maths.MaxAffineTransport.RationalLabel.toLabel`: rational max-affine labels
  and their cast to real labels.
* `Maths.MaxAffineTransport.rationalBranchDelta` and
  `Maths.MaxAffineTransport.rationalBranchBase`: the rational branch system.
* `Maths.MaxAffineTransport.IsRationalLaxSection`: a lax section with rational
  values.

## Main results

* `Maths.MaxAffineTransport.rationalBranchDelta_cast` and
  `Maths.MaxAffineTransport.rationalBranchBase_cast`: the rational branch
  system casts to the real one.
* `Maths.MaxAffineTransport.exists_rationalLaxSection_iff_exists_realLaxSection`:
  real feasibility is equivalent to rational feasibility.
* `Maths.MaxAffineTransport.exists_integralBranchCertificate_of_real_infeasible`:
  real infeasibility has an integer-valued branch certificate.
* In the namespace `Maths.MaxAffineTransport`, the results
