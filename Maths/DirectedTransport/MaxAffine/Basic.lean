...
`Maths.DirectedTransport.MaxAffine.Additive` identifies unit-slope
...
`Maths.DirectedTransport.Basic` is the generic semantic layer.  `toTransport`
...
`Maths.DirectedTransport.MaxAffine.FixedPoint`; its eigenvectors below unit slope
modulus, where the operator contracts, the subject of
`Maths.DirectedTransport.MaxAffine.Contraction`; and its eigenvectors at nonnegative
...
`Maths.DirectedTransport.MaxAffine.Eigenproblem`.  What is settled there is existence
and not the value of the eigenvalue; the **least** eigenvalue is the optimum
of a finite linear program, by `Maths.DirectedTransport.MaxAffine.LeastEigenvalue`.
...
quantitative obstruction.  `Maths.DirectedTransport.MaxAffine.Sections` proves section
existence in the subunit-slope and slope-one regimes;
`Maths.DirectedTransport.MaxAffine.Farkas` gives the finite linear-system alternative for
general labels.
