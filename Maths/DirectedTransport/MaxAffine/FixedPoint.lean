...
import Maths.DirectedTransport.Additive.CycleMean
...
`Maths.MaxAffineTransport.IsLaxSection` holds for `x` if and only if `F x ≤ x`
...
`Maths.MaxPlusPotential.maxRootedWeight` -- the column of the Kleene star of
...
modulus `Maths.DirectedTransport.MaxAffine.Contraction` pins one down uniquely, for every real
eigenvalue, and at nonnegative slopes straddling one
`Maths.DirectedTransport.MaxAffine.Eigenproblem` produces one by Brouwer's theorem in exponential
coordinates, floors included, with
`Maths.MaxAffineTransport.not_exists_isEigenvector_flipLabel` showing the sign
...
eigenvalues is in `Maths.DirectedTransport.MaxAffine.Spectrum`.

## Main definitions

* `Maths.MaxAffineTransport.incoming`: the edges arriving at a vertex.
* `Maths.MaxAffineTransport.vertexOperatorBot`,
  `Maths.MaxAffineTransport.vertexOperator`: the vertex operator, valued in
  `WithBot ℝ` so that a vertex with no incoming edge is meaningful, and its real-valued form.
* `Maths.MaxAffineTransport.IsEigenvector`: the equation `F x = lam + x`.

## Main results

* `Maths.MaxAffineTransport.isLaxSection_iff_vertexOperatorBot_le` and
  `Maths.MaxAffineTransport.isLaxSection_iff_vertexOperator_le`: **a lax section is
  precisely a sub-fixed point of the vertex operator.**
* `Maths.MaxAffineTransport.monotone_vertexOperator`: monotonicity at nonnegative
  slopes.
* `Maths.MaxAffineTransport.vertexOperator_add_const_le`: subhomogeneity for slopes
  in `[0, 1]`.
* `Maths.MaxAffineTransport.vertexOperator_add_const`: additive homogeneity for
  unit-slope floorless labels, which with monotonicity makes the operator topical
  (`Maths.MaxAffineTransport.monotone_vertexOperator_of_translation`).
* `Maths.MaxAffineTransport.isEigenvector_iff`: the eigenvalue equation splits into
  the edgewise inequality and tightness at every vertex.
* `Maths.MaxAffineTransport.isEigenvector_maxRootedWeight_of_maximizing_cycle`:
...
* `Maths.MaxAffineTransport.exists_isEigenvector_of_translation`: over the reals, on
...
`Maths.MaxAffineTransport.vertexOperator` takes the junk value `0` at a vertex with
no incoming edge, in the manner of `Maths.MaxPlusPotential.maxRootedWeight`; the
`WithBot`-valued `Maths.MaxAffineTransport.vertexOperatorBot` is the honest object
...
`Maths.DirectedTransport.MaxAffine.Farkas` and `Maths.DirectedTransport.MaxAffine.Slopes` provide.
...
Lipschitz estimate `Maths.MaxAffineTransport.Label.apply_add_le` read at a vertex. -/
...
`Maths.MaxAffineTransport.monotone_vertexOperator` and
`Maths.MaxAffineTransport.vertexOperator_add_const`, and both hypotheses of the
...
`Maths.MaxPlusPotential.isEigenvector_iff`. -/
