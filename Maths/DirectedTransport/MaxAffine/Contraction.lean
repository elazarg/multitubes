...
`Maths.DirectedTransport.MaxAffine.Eigenproblem` by Brouwer's theorem in exponential coordinates,
...
eigenvector are both non-unique, by `Maths.DirectedTransport.MaxAffine.Spectrum`, where the residual
