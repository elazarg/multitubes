...
import Maths.DirectedTransport.FiniteInequality.Arithmetic
...
* `Maths.FiniteInequality.balanceMatrix`: the row data read as a matrix of balance
  columns indexed by rows.
* `Maths.FiniteInequality.augmentedColumn`: a balance column extended by the
  mass-one coordinate, so that normalization becomes one more linear condition.
* `Maths.FiniteInequality.IsPositiveCircuit`: a normalized certificate whose
...
* `Maths.FiniteInequality.exists_positive_normalizedCertificate_of_infeasible`:
  infeasibility produces a normalized certificate of positive objective.
* `Maths.FiniteInequality.exists_positive_normalizedCertificate_linearIndependent`:
...
* `Maths.FiniteInequality.exists_positiveCircuit_of_infeasible`: the same witness
  packaged as a minimal affine row configuration around the origin.
* `Maths.FiniteInequality.exists_positive_normalizedCertificate_support_card_le_rank`
...
* `Maths.FiniteInequality.exists_rankSparse_rationalCertificate_of_real_infeasible`:
...
theorem mem_normalizedFarkasCertificateSet_iff
    ...
    coefficient ∈ LinearProgramming.normalizedFarkasCertificateSet
...
private theorem normalizedCertificateSet_isCompact
  ...
  have hclosed : IsClosed feasible :=
    LinearProgramming.isClosed_standardFeasibleSet _ _
...
theorem exists_positive_normalizedCertificate_linearIndependent
  ...
  have hstandardOptimal : LinearProgramming.IsStandardOptimal
      (LinearProgramming.normalizedFarkasMatrix
        (balanceMatrix delta) (fun _ => 1))
  ...
  obtain ⟨extreme, hextreme, hextremeOptimal, hvalue⟩ :=
    LinearProgramming.exists_extreme_standardOptimal_of_standardOptimal
      (LinearProgramming.normalizedFarkasMatrix
        (balanceMatrix delta) (fun _ => 1))
      LinearProgramming.normalizedFarkasRhs base hstandardOptimal
  ...
  have hlinear :=
    LinearProgramming.linearIndependent_supportColumns_of_extreme_standardFeasible
      (LinearProgramming.normalizedFarkasMatrix
        (balanceMatrix delta) (fun _ => 1))
  ...
  have haugmented :
    ...
    | inl state =>
        ...
        simpa [Finset.sum_apply, Pi.smul_apply, ite_apply, augmentedColumn,
          Matrix.col_apply, LinearProgramming.normalizedFarkasMatrix,
...
theorem exists_rankSparse_rationalCertificate_of_real_infeasible
  ...
  have hselectedInfeasible : ¬∃ potential : State → ℝ,
    ...
    have hvalue : 0 <
      ...
      exact hpositive
    have hweak := Maths.not_nonnegative_incompatibility_of_potential
