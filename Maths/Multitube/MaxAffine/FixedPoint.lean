...
import Maths.Multitube.Additive.CycleMean
import Maths.Multitube.Additive.ShortCycles
...
modulus `Maths.Multitube.MaxAffine.Contraction` pins one down uniquely, for every real
eigenvalue, and at nonnegative slopes straddling one
`Maths.Multitube.MaxAffine.Eigenproblem` produces one by Brouwer's theorem in exponential
...
eigenvalues is in `Maths.Multitube.MaxAffine.Spectrum`.
...
`Maths.Multitube.MaxAffine.Farkas` and `Maths.Multitube.MaxAffine.Slopes` provide.
