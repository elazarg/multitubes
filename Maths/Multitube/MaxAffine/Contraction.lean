...
`Maths.Multitube.MaxAffine.FixedPoint` solves the eigenvalue equation `F x = lam + x` for
...
`Maths.Multitube.MaxAffine.Eigenproblem` by Brouwer's theorem in exponential coordinates,
...
eigenvector are both non-unique, by `Maths.Multitube.MaxAffine.Spectrum`, where the residual
