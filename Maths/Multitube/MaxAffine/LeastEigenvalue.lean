...
`gamma_b + alpha_b * x (sigma_b) - x (tau_b) ≤ lam` of `Maths.Multitube.MaxAffine.Duality`,
...
The branch system is the one of `Maths.Multitube.MaxAffine.Duality` not a fresh
