...
`Maths.Multitube.MaxAffine.Spectrum` on one vertex carrying a reset of slope `0` and a
...
`Maths.Multitube.MaxAffine.LeastEigenvalue` a least such level is the least eigenvalue
