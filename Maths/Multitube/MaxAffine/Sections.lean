...
`Maths.Multitube.MaxAffine.Farkas` settles them by a linear test, not a cyclewise
...
settled in `Maths.Multitube.MaxAffine.FixedPoint`.  Mixed slopes below unit
modulus are settled metrically in `Maths.Multitube.MaxAffine.Contraction`,
...
settled topologically in `Maths.Multitube.MaxAffine.Eigenproblem`: an
