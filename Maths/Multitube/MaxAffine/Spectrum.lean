...
criteria of `Maths.Multitube.MaxAffine.Slopes`.  Those two are one criterion read along
...
incoming edge, and so that the criteria of `Maths.Multitube.MaxAffine.Slopes` apply to it
...
relaxation level is the least eigenvalue, by `Maths.Multitube.MaxAffine.LeastEigenvalue`, so
