...
`Maths.Multitube.MaxAffine.Additive` identifies unit-slope
...
`Maths.Multitube.Basic` is the generic semantic layer.  `toTransport`
...
`Maths.Multitube.MaxAffine.FixedPoint`; its eigenvectors below unit slope
modulus, where the operator contracts, the subject of
`Maths.Multitube.MaxAffine.Contraction`; and its eigenvectors at nonnegative
...
`Maths.Multitube.MaxAffine.Eigenproblem`.  What is settled there is existence
and not the value of the eigenvalue; the **least** eigenvalue is the optimum
of a finite linear program, by `Maths.Multitube.MaxAffine.LeastEigenvalue`.
...
quantitative obstruction.  `Maths.Multitube.MaxAffine.Sections` proves section
existence in the subunit-slope and slope-one regimes;
`Maths.Multitube.MaxAffine.Farkas` gives the finite linear-system alternative for
general labels.
