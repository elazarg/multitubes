...
retract theories: one-base flatness of a transport (`Maths.Multitube.Exact`)
...
one, so the path-functor translation of `Maths.Multitube.Category` carries an
...
* `Maths.Transport.compressed_walkMap_eq_via_category`: the concrete retract
  normal form of `Maths.Multitube.Exact`, rederived through the path-functor dictionary.
-/
...
`Maths.Multitube.Exact` proves the retract normal form concretely, by walk
induction, and `Maths.Multitube.CategoricalRetracts` proves it categorically.
