...
spectral theory.  The equality side is `Maths.Multitube.Additive.Eigenvector`,
and the attained cycle-mean characterization is
`Maths.Multitube.Additive.CycleMean`.  The critical graph as a subgraph in its
...
`Maths.Multitube.Additive.CriticalGraph`, as are the strongly connected
