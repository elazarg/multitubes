...
`Maths.Multitube.Additive.Eigenvector` produces one max-plus eigenvector for the maximum
...
`Maths.Multitube.Additive.Condensation`; both are one-line reindexings of an ambient graph
