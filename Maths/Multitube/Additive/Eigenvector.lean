...
`Maths.Multitube.Additive.Potentials` produces *sub*eigenvectors: functions with
...
Following `Maths.Multitube.Additive.CycleMean`, the general-field theorem takes the
...
vertices, are `Maths.Multitube.Additive.CriticalGraph`.  So are the critical *classes* - the
