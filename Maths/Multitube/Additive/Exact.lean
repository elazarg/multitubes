...
`Maths.Multitube.Additive.Potentials` develops the one-sided inequality
...
`Maths.Multitube.Basic`, and the reachability scopes
...
the monoid walk label in `Maths.Multitube.Basic`; this section relates
it to the additive walk weight and records its subtraction rule. -/

/-- The walk weight of `Maths.Multitube.Additive.Potentials` is the
