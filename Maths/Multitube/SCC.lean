...
/-!
# Strongly connected normal forms for transport
...
* `Maths.ReachabilityVertex`: the vertex type carrying the reachability
  preorder.
* `Maths.SCC` and `Maths.toSCC`: the strongly connected components
  as the antisymmetrization of that preorder, and the quotient map.
* `Maths.InterSCCEdge` and `Maths.sccCondensation`: the acyclic
  condensation graph on components.
* `Maths.Transport.HasSCCTrivializationAt` and
  `Maths.Transport.HasSCCTrivializations`: fiberwise equivalences identifying
  every edge map inside a component with a change of coordinates.
* `Maths.Transport.sccTrivialization`: the trivialization built from trivial
  holonomy.

## Main results

* `Maths.sccCondensation_edge_strict` and
  `Maths.sccCondensation_no_nonempty_closedWalk`: the condensation is acyclic.
* `Maths.Transport.hasTrivialHolonomy_iff_hasSCCTrivializations`: trivial
