...
* `Maths.TransferSummary.AffineSummary` and its
  `Maths.TransferSummary.AffineSummary.instMonoid` instance.
* `Maths.TransferSummary.MaxAffineSummary` and
  `Maths.TransferSummary.MaxAffineSummary.comp`.
* `Maths.TransferSummary.reflectedIter` -- the reflected orbit.
* `Maths.TransferSummary.transport`,
  `Maths.TransferSummary.service`,
  `Maths.TransferSummary.lindleyTerm` -- the three ingredients of Lindley's
  representation.

## Main results

* `Maths.TransferSummary.AffineSummary.apply_comp`,
  `Maths.TransferSummary.AffineSummary.comp_assoc`,
  `Maths.TransferSummary.AffineSummary.apply_pow` -- affine words keep two
  coefficients.
* `Maths.TransferSummary.AffineSummary.monotone_apply_iff` -- monotone exactly at
  nonnegative slope.
* `Maths.TransferSummary.MaxAffineSummary.apply_comp`,
  `Maths.TransferSummary.MaxAffineSummary.comp_assoc`,
  `Maths.TransferSummary.MaxAffineSummary.apply_foldr` -- max-affine words keep
  three coefficients.
* `Maths.TransferSummary.MaxAffineSummary.monotone_apply_iff` and
  `Maths.TransferSummary.MaxAffineSummary.not_exists_apply_eq_id`.
* `Maths.TransferSummary.reflectedIter_eq_sup'` -- Lindley's representation.
* `Maths.TransferSummary.monotone_reflectedIter` and
  `Maths.TransferSummary.reflectedIter_le_of_le_service` -- monotone in the
  initial state, antitone in the service sequence.
* `Maths.TransferSummary.reflectedIter_add` -- the orbit restarts from its own
  state, and hence `Maths.TransferSummary.reflectedIter_eq_of_eq_zero`: once the
  ...
  `Maths.TransferSummary.reflectedIter_eq_sup'_of_eq_zero`.
...
extension is `Maths.MaxAffineTransport.Label`, whose monoid instance on the
nonnegative-slope labels, action on the line, and two embeddings
`Maths.MaxAffineTransport.Label.ofAffine` and
`Maths.MaxAffineTransport.Label.ofMaxAffine` are stated there and not here: the
...
`Maths.TransferSummary.tendsto_loynes_ciSup`. That statement is strictly weaker than
