...
* `Maths.InverseCoordinate.affineStep`,
  `Maths.InverseCoordinate.affineFixedPoint`: the affine recurrence and its fixed
  point.
* `Maths.InverseCoordinate.linearFractionalStep`: the conjugate Möbius step.
* `Maths.InverseCoordinate.IsHazardBalance`,
  `Maths.InverseCoordinate.hazardCeiling`,
  `Maths.InverseCoordinate.hazardOrbit`: the worked family.
* `Maths.InverseCoordinate.affineTransferMatrix`,
  `Maths.InverseCoordinate.linearFractionalTransferMatrix`,
  `Maths.InverseCoordinate.inversionMatrix`,
  `Maths.InverseCoordinate.linearFractionalGL`: the matrix reading.
* `Maths.InverseCoordinate.affineCycleSlope`,
  `Maths.InverseCoordinate.affineCycleShift`,
  `Maths.InverseCoordinate.affineCycleStep`,
  `Maths.InverseCoordinate.affineCycleMatrix`: the `n`-phase cycle.
* `Maths.InverseCoordinate.projectiveDiscriminant`,
  `Maths.InverseCoordinate.IsProjectiveHyperbolic`,
  `Maths.InverseCoordinate.IsProjectiveParabolic`,
  `Maths.InverseCoordinate.IsProjectiveElliptic`: the classification.
* `Maths.InverseCoordinate.projectiveStep`,
  `Maths.InverseCoordinate.projectiveFixedPolynomial`,
  `Maths.InverseCoordinate.projectiveFixedPoint`,
  `Maths.InverseCoordinate.projectiveGL`: the projective action of a `2 × 2` matrix
  and its fixed points.
* `Maths.InverseCoordinate.quarterTurnMatrix`: the elliptic witness.

## Main results

* `Maths.InverseCoordinate.affine_eq_closedForm`: the closed form of the affine
  recurrence.
* `Maths.InverseCoordinate.linearFractionalStep_eq_inv_affineStep_inv`: the
  inverse-coordinate conjugacy.
* `Maths.InverseCoordinate.IsHazardBalance.tendsto_zero_iff`: classification of the
  vanishing solutions of the worked family.
* `Maths.InverseCoordinate.affineTransferMatrix_mul`,
  `Maths.InverseCoordinate.linearFractionalTransferMatrix_mul`: composition is matrix
  multiplication.
* `Maths.InverseCoordinate.linearFractionalGL_smul_coe`,
  `Maths.InverseCoordinate.linearFractionalGL_cycle_smul`: the projective action.
* `Maths.InverseCoordinate.affineCycleMatrix_eq`,
  `Maths.InverseCoordinate.affineCycleStep_eq`: an `n`-phase affine cycle is a single
  affine step, at matrix and at scalar level.
* `Maths.InverseCoordinate.isProjectiveHyperbolic_or_parabolic_or_elliptic`: the
  trichotomy.
* `Maths.InverseCoordinate.not_isProjectiveElliptic_affineCycleMatrix`,
  `Maths.InverseCoordinate.isProjectiveHyperbolic_affineCycleMatrix_iff`,
  `Maths.InverseCoordinate.isProjectiveParabolic_affineCycleMatrix_iff`: the affine
  degeneration of the classification.
* `Maths.InverseCoordinate.affineCycleStep_eq_self_iff_of_slope_ne_one`,
  `Maths.InverseCoordinate.affineCycleStep_eq_self_iff_of_slope_one`: the periodic
  seeds of a cycle in each case.
* `Maths.InverseCoordinate.discrim_projectiveFixedPolynomial`: the discriminant of the
  fixed-point quadratic is `projectiveDiscriminant`.
* `Maths.InverseCoordinate.projectiveGL_smul_coe_eq_self_iff`: the fixed points of the
  projective action on the affine chart are the roots of the quadratic.
* `Maths.InverseCoordinate.projectiveGL_smul_eq_self_iff_of_isProjectiveHyperbolic`,
  `Maths.InverseCoordinate.projectiveGL_smul_eq_self_iff_of_isProjectiveParabolic`,
  `Maths.InverseCoordinate.projectiveGL_smul_ne_self_of_isProjectiveElliptic`: two
  fixed points, one, and none.
* `Maths.InverseCoordinate.exists_isProjectiveElliptic`: the elliptic case is
