...
* `Maths.EdgeGraph.Walk.IsTrailWithin`: a walk with pairwise distinct edges, all
  drawn from a prescribed finite edge set.
* `Maths.EdgeGraph.MultiplicityToken`: the distinguishable copies of an edge under
  a nonnegative integer multiplicity.
* `Maths.EdgeGraph.tokenGraph`: the multigraph on those tokens.
* `Maths.EdgeGraph.Walk.detokenize`: forget token indices along a walk.

## Main results

* `Maths.EdgeGraph.Walk.exists_maximalTrailWithin`: a longest allowed trail from a
  prescribed vertex exists.
* `Maths.EdgeGraph.Walk.maximalTrailWithin_isClosed`: in a balanced edge set a
  longest trail returns to its start.
* `Maths.EdgeGraph.Walk.exists_closedTrail_covering_of_balanced_connected`: the
  directed Euler theorem for a balanced, weakly connected finite edge set.
* `Maths.EdgeGraph.ConnectedIntegerCirculation.exists_closedWalk_exactMultiplicity_at`:
