...
An infinite walk of an `Maths.EdgeGraph` is a sequence of edges whose consecutive
...
* `Maths.EdgeGraph.CyclicWord`: a nonempty closed edge word based at a vertex.
* `Maths.EdgeGraph.InfiniteWalk`: an infinite directed walk from a vertex.
...
* `Maths.EdgeGraph.CyclicWord.toInfiniteWalk`,
  `Maths.EdgeGraph.Walk.prependInfinite`: repetition of a cyclic word, and
...
* `Maths.EdgeGraph.InfiniteWalk.segment_charge`: the charge of a finite segment is
