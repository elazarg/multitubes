...
* `DirectedTransport.TransferSummary.AffineSummary.fixedPoint` -- the point
  `shift / (1 - slope)`.

## Main results

* `DirectedTransport.TransferSummary.AffineSummary.apply_fixedPoint` and
  `DirectedTransport.TransferSummary.AffineSummary.eq_fixedPoint_of_apply_eq` -- at slope `≠ 1`
  the fixed point is fixed and is the only fixed point, packaged as
  `DirectedTransport.TransferSummary.AffineSummary.existsUnique_apply_eq`.
* `DirectedTransport.TransferSummary.AffineSummary.apply_sub_fixedPoint` -- the displacement
  from the fixed point is multiplied by the slope.
* `DirectedTransport.TransferSummary.AffineSummary.iterate_apply` and
  `DirectedTransport.TransferSummary.AffineSummary.apply_pow_eq` -- the closed form of the
  orbit.
* `DirectedTransport.TransferSummary.AffineSummary.tendsto_iterate_apply` -- convergence to the
  fixed point at slope of absolute value `< 1`.
* `DirectedTransport.TransferSummary.AffineSummary.apply_of_slope_eq_one`,
  `DirectedTransport.TransferSummary.AffineSummary.iterate_apply_of_slope_eq_one` and
  `DirectedTransport.TransferSummary.AffineSummary.exists_apply_eq_iff_of_slope_eq_one` -- the
...
namespace DirectedTransport.TransferSummary.AffineSummary
...
end DirectedTransport.TransferSummary.AffineSummary
