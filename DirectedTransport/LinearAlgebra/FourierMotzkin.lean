...
* `DirectedTransport.LinearAlgebra.rowEval`: the left-hand side of one row at a point.
* `DirectedTransport.LinearAlgebra.IsFeasible`: primal feasibility of `A x ≥ b`.
* `DirectedTransport.LinearAlgebra.IsCertificate`,
  `DirectedTransport.LinearAlgebra.HasCertificate`: Farkas certificates of infeasibility.
* `DirectedTransport.LinearAlgebra.FMRowIndex`, `DirectedTransport.LinearAlgebra.fmA`,
  `DirectedTransport.LinearAlgebra.fmB`: the Fourier-Motzkin reduced system.
* `DirectedTransport.LinearAlgebra.liftCoeff`, `DirectedTransport.LinearAlgebra.liftCert`:
  the lift of a reduced certificate to the original system.
* `DirectedTransport.LinearAlgebra.HasBalancedCertificate`: a nonzero nonnegative vector
  annihilating every column, the dual alternative in Gordan's transposition theorem.

## Main results

* `DirectedTransport.LinearAlgebra.feas_cert_disjoint`: feasibility and a certificate cannot
  both hold.
* `DirectedTransport.LinearAlgebra.fm_feasible_of_feasible`,
  `DirectedTransport.LinearAlgebra.feasible_of_fm_feasible`: feasibility transfers along the
  Fourier-Motzkin reduction in both directions.
* `DirectedTransport.LinearAlgebra.fm_cert_lift`: a certificate of the reduced system lifts.
* `DirectedTransport.LinearAlgebra.theorem_of_alternative`: the theorem of the alternative.
* `DirectedTransport.LinearAlgebra.exists_rowEval_pos_iff_not_hasBalancedCertificate`:
...
`A x ≥ 1` is, and `DirectedTransport.LinearAlgebra.theorem_of_alternative` at `b = 1` turns the
