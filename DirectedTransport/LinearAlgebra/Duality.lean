...
Following the pattern of `DirectedTransport.MaxAffine.Duality` and
`DirectedTransport.FiniteInequality.worstResidualAtMost_iff_normalizedDual_le`, strong duality
...
`DirectedTransport.LinearAlgebra.exists_scaledDual_of_not_exists_objective_ge` is the sharper,
...
* `DirectedTransport.LinearAlgebra.objective_le_of_isDualFeasible`: **weak duality**.
* `DirectedTransport.LinearAlgebra.exists_scaledDual_of_not_exists_objective_ge`: an
  unreachable level is witnessed by a scaled dual vector.
* `DirectedTransport.LinearAlgebra.exists_objective_ge_iff_feasible_and_forall_isDualFeasible`,
...
Together with `DirectedTransport.LinearAlgebra.objective_le_of_isDualFeasible` this pins the
...
`DirectedTransport.LinearAlgebra.exists_isDualFeasible_objective_le_of_forall_le`. -/
