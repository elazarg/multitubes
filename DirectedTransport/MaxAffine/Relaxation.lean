...
* `DirectedTransport.MaxAffineTransport.Label.pathRelaxationMass`: the suffix-product mass
  by which a uniform relaxation moves the composite shift.
* `DirectedTransport.MaxAffineTransport.Label.expansiveResetThreshold`,
  `DirectedTransport.MaxAffineTransport.Label.expansiveResetThresholds`,
  `DirectedTransport.MaxAffineTransport.Label.thresholdSup`: the per-reset thresholds of a
  superunit-product cycle and their supremum in `WithBot ℝ`.
* `DirectedTransport.MaxAffineTransport.Label.expansiveCyclicSlackThreshold` and
  `DirectedTransport.MaxAffineTransport.Label.cyclicSlackThreshold`: the exact cycle-slack
  threshold, `⊥` when every level is feasible.

## Main results

* `DirectedTransport.MaxAffineTransport.Label.pathShift_map_relax` and
  `DirectedTransport.MaxAffineTransport.Label.pathSlope_map_relax`: uniform relaxation
  preserves the path slope and subtracts `level` times the relaxation mass from the shift.
* `DirectedTransport.MaxAffineTransport.Label.one_le_pathRelaxationMass`: a nonempty path
  with nonnegative slopes has relaxation mass at least one.
* `DirectedTransport.MaxAffineTransport.Label.hasCyclicSlack_iff_cycleMean_le_of_pathSlope_eq_one`:
  a unit-product cycle attains the threshold `pathShift / pathRelaxationMass`.
* `DirectedTransport.MaxAffineTransport.Label.hasCyclicSlack_of_pathSlope_lt_one` and
  `DirectedTransport.MaxAffineTransport.Label.hasCyclicSlack_of_one_lt_pathSlope_of_floorless`:
  the subunit and floorless superunit escape cases.
* `DirectedTransport.MaxAffineTransport.Label.hasCyclicSlack_iff_cyclicSlackThreshold_le` and
  `DirectedTransport.MaxAffineTransport.Label.cyclicSlackThreshold_eq_bot_iff_all_levels`:
