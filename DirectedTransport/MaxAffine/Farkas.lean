...
import DirectedTransport.FiniteInequality.Basic

/-!
# Lax sections at mixed slopes: the cyclewise test fails, the linear test does not

`DirectedTransport.MaxAffine.Sections` settles existence of a lax section in
...
`DirectedTransport.MaxAffineTransport.Label.exists_apply_le_self_iff`.  A walk using the
...
splitting of `DirectedTransport.MaxAffineTransport.Label.apply_le_target_iff`.  Whatever the
...
`DirectedTransport.exists_potential_iff_no_nonnegative_incompatibility`.

## Main definitions

* `DirectedTransport.MaxAffineTransport.loopGraph`,
  `DirectedTransport.MaxAffineTransport.resetLabel`,
  `DirectedTransport.MaxAffineTransport.doubleLabel`,
  `DirectedTransport.MaxAffineTransport.loopLabel` -- the one-vertex two-loop
  counterexample.
* `DirectedTransport.MaxAffineTransport.rowDelta`,
  `DirectedTransport.MaxAffineTransport.rowBase` -- the two linear inequality rows
  contributed by each edge.
* `DirectedTransport.MaxAffineTransport.IsFarkasCertificate` -- a nonnegative balanced
  combination of those rows with positive total bound.

## Main results

* `DirectedTransport.MaxAffineTransport.isLaxSection_iff_forall_row` -- lax sections are
  exactly the solutions of the row system.
* `DirectedTransport.MaxAffineTransport.not_exists_isLaxSection_loopLabel` -- the two-loop
  labelling has no lax section.
* `DirectedTransport.MaxAffineTransport.exists_apply_compList_le_self_loopLabel` -- every
  closed walk of that graph has a composite label with a pre-fixed point.
* `DirectedTransport.MaxAffineTransport.not_forall_cycle_prefixed_imp_exists_isLaxSection` --
  the two together: the cyclewise test is not sufficient, even for finitely many
  vertices and edges and nonnegative slopes.
* `DirectedTransport.MaxAffineTransport.exists_isLaxSection_iff_no_nonnegative_certificate` --
...
the inner floor pushed forward, and `DirectedTransport.MaxAffineTransport.Label.pushFloor`
preserves `⊥`. -/
...
`DirectedTransport.MaxAffineTransport.Label.exists_apply_le_self_iff` outright, so only the
...
/-- The two loops of `DirectedTransport.MaxAffineTransport.loopGraph`: `false` resets and
`true` doubles. -/
...
`DirectedTransport.MaxAffineTransport.Label.exists_apply_le_self_of_floor_bot` reduces the
question to the unit-slope case, which by
`DirectedTransport.MaxAffineTransport.slopeProd_loopLabel_cases` occurs only for the empty
...
labelling of `DirectedTransport.MaxAffineTransport.loopLabel` has nonnegative slopes on a
...
`DirectedTransport.MaxAffineTransport.exists_isLaxSection_iff_forall_cycle_exists_prefixed`;
`DirectedTransport.MaxAffineTransport.exists_isLaxSection_iff_no_nonnegative_certificate` is
...
`DirectedTransport.MaxAffineTransport.rowBase` makes vacuous. -/
...
`DirectedTransport.MaxAffineTransport.Label.apply_le_target_iff`. -/
...
bound.  This is
`DirectedTransport.exists_potential_iff_no_nonnegative_incompatibility` at the encoding of
`DirectedTransport.MaxAffineTransport.rowDelta` and
`DirectedTransport.MaxAffineTransport.rowBase`, so no sign condition on the slopes and no
...
theorem exists_isLaxSection_iff_no_nonnegative_certificate [Fintype E]
  ...
  exact DirectedTransport.exists_potential_iff_no_nonnegative_incompatibility
