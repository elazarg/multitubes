...
* `DirectedTransport.MaxAffineTransport.isLaxSection_translationLabel_iff`: a lax section for
  translation labels is exactly an additive potential.
* `DirectedTransport.MaxAffineTransport.defect_translationLabel`: the max-affine defect of a
  translation label is the additive defect of its weight.
* `DirectedTransport.MaxAffineTransport.isCoboundary_iff_exists_translation_defect_eq_zero`:
  edge weights are a coboundary exactly when some translation-labelled candidate has zero
  defect on every edge.
* `DirectedTransport.MaxAffineTransport.holonomyApply_translationLabel`: translation transport
...
theorem isLaxSection_translationLabel_iff
    ...
    IsLaxSection G (fun edge => translationLabel (weight edge)) potential ↔
      DirectedTransport.MaxPlusPotential.IsPotential G weight potential := by
  simp only [IsLaxSection, DirectedTransport.MaxPlusPotential.IsPotential, apply_translationLabel]
...
@[simp] theorem defect_translationLabel
    ...
    defect G (fun next => translationLabel (weight next)) potential edge =
      DirectedTransport.MaxPlusPotential.defect G weight potential edge := by
  simp only [defect, DirectedTransport.MaxPlusPotential.defect, apply_translationLabel]
  ring
...
theorem isCoboundary_iff_exists_translation_defect_eq_zero
    ...
    DirectedTransport.CycleCoboundary.IsCoboundary G weight ↔
      ∃ potential : V → ℝ,
        ∀ edge : E,
          defect G (fun next => translationLabel (weight next)) potential edge = 0 := by
  rw [DirectedTransport.CycleCoboundary.isCoboundary_iff_exists_defect_eq_zero]
...
theorem holonomyApply_translationLabel
    ...
    holonomyApply (fun edge => translationLabel (weight edge)) walk point =
      point + DirectedTransport.MaxPlusPotential.walkWeight weight walk := by
  induction walk with
  | nil => simp
  | concat walkSoFar edge legal ih =>
      rw [holonomyApply_concat, ih, apply_translationLabel,
        DirectedTransport.MaxPlusPotential.walkWeight_concat]
      ring
