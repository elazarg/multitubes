...
`DirectedTransport.MaxAffineTransport.Label.compList_eq_pathCoefficients` and the complete
scalar classification computes every finite cycle without introducing a fake real value for
`-∞`.

## Main definitions

* `DirectedTransport.MaxAffineTransport.Label.relax`: a label with a common slack subtracted
  from both branches.
* `DirectedTransport.MaxAffineTransport.Label.IsSlackChain` and
  `DirectedTransport.MaxAffineTransport.Label.HasCyclicSlack`: the cyclic slack problem.
* `DirectedTransport.MaxAffineTransport.Label.relaxedOrbitSteps` and
  `DirectedTransport.MaxAffineTransport.Label.closeLastTarget`: the canonical chain
  realizing a slack level, and its closure at a chosen terminal value.

## Main results

* `DirectedTransport.MaxAffineTransport.Label.hasCyclicSlack_iff_compList_relaxed_prefixed`:
  cyclic slack is a pre-fixed-point problem for one composite relaxed label.
* `DirectedTransport.MaxAffineTransport.Label.hasCyclicSlack_iff_pathCoefficient_prefixed`:
  the same test read off the path coefficients.
* `DirectedTransport.MaxAffineTransport.Label.hasCyclicSlack_iff_coefficient_trichotomy`:
