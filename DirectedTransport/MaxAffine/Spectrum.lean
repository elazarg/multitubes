...
`DirectedTransport.MaxAffineTransport.exists_isEigenvector_of_slope_nonneg` produces an
...
`DirectedTransport.MaxAffineTransport.eigenvalues G label = {lam | ∃ x, F x = lam + x}`
...
`DirectedTransport.MaxAffineTransport.relaxationLevels`, the real numbers `lam` for which the
relaxed labelling `DirectedTransport.MaxAffineTransport.Label.relax lam ∘ label` admits a lax
...
In the two regimes where the sub-fixed-point theory is complete the up-set is described by
cycles.  For slopes at most one, and for slopes at least one with floors absent, a lax section
exists exactly when every closed walk of the unit-slope subgraph has nonpositive shift sum, by the
two cycle criteria of `DirectedTransport.MaxAffine.Slopes`.  Relaxing by `lam` subtracts `lam` from
...
`DirectedTransport.MaxAffineTransport.loopLabel`.  Its vertex operator is `t ↦ max 10 (2 * t)`,
...
(`DirectedTransport.MaxAffineTransport.isEigenvector_add_const_of_slope_eq`), which combined with
...
* `DirectedTransport.MaxAffineTransport.eigenvalues`: the set of eigenvalues of the vertex
  operator.
* `DirectedTransport.MaxAffineTransport.relaxationLevels`: the set of levels at which the
  relaxed labelling has a lax section, equivalently the solvable levels of `F x ≤ lam + x`.

## Main results

* `DirectedTransport.MaxAffineTransport.eigenvalues_subset_relaxationLevels` and
  `DirectedTransport.MaxAffineTransport.mem_relaxationLevels_of_le`: **the eigenvalues lie in an
  up-set of relaxation levels**, with no hypothesis on the slopes.
* `DirectedTransport.MaxAffineTransport.mem_relaxationLevels_iff_unitSlopeCycles_of_slope_le_one`
  and
  `DirectedTransport.MaxAffineTransport.mem_relaxationLevels_iff_unitSlopeCycles_of_one_le_slope`:
  **the relaxation levels are the upper bounds of the unit-slope cycle means**, in the subunit and
  superunit regimes.
* `DirectedTransport.MaxAffineTransport.unitSlopeCycles_le_of_isEigenvector_of_slope_le_one` and
  `DirectedTransport.MaxAffineTransport.unitSlopeCycles_le_of_isEigenvector_of_one_le_slope`:
  **every eigenvalue dominates every unit-slope cycle mean** in those regimes.
* `DirectedTransport.MaxAffineTransport.eigenvalues_eq_univ_of_slope_eq`: **at a common
  nonnegative slope other than one every real number is an eigenvalue.**
* `DirectedTransport.MaxAffineTransport.eq_zero_of_isEigenvector_add_const_of_slope_eq`: at a
  common slope other than one, eigenvectors of one eigenvalue never differ by a nonzero constant.
* `DirectedTransport.MaxAffineTransport.eigenvalues_loopLabel`: **the two-loop labelling has
...
* `DirectedTransport.MaxAffineTransport.exists_pair_ne_isEigenvector_loopLabel`: above the
  threshold that labelling has two distinct eigenvectors for one eigenvalue.
* `DirectedTransport.MaxAffineTransport.relaxationLevels_loopLabel`: for that labelling the
...
`DirectedTransport.MaxAffineTransport.relaxationLevels` is defined through
`DirectedTransport.MaxAffineTransport.Label.relax` rather than by an inequality on the vertex
operator, so that it needs no finiteness of the edge type and no hypothesis that a vertex has an
incoming edge, and so that the criteria of `DirectedTransport.MaxAffine.Slopes` apply to it
verbatim.  `DirectedTransport.MaxAffineTransport.mem_relaxationLevels_iff` is the elementary
edgewise reading, and `DirectedTransport.MaxAffineTransport.eigenvalues_subset_relaxationLevels`
...
eigenvector exists by `DirectedTransport.MaxAffineTransport.exists_isEigenvector_of_slope_nonneg`,
...
/-- The vertex operator of `DirectedTransport.MaxAffineTransport.loopLabel`: the better of the
...
`DirectedTransport.MaxAffineTransport.eq_zero_of_isEigenvector_add_const_of_slope_eq`, and only
at a common slope. -/
...
/-- No loop of `DirectedTransport.MaxAffineTransport.loopGraph` has unit slope: the reset has
