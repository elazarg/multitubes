...
* `DirectedTransport.MaxAffineJoinAdapter.NonnegativeAffine`,
  `DirectedTransport.MaxAffineJoinAdapter.NonnegativeLabel`,
  `DirectedTransport.MaxAffineJoinAdapter.AbstractLabel`: the three carriers being related.
* `DirectedTransport.MaxAffineJoinAdapter.instSupBotMulAction`: nonnegative affine summaries
  acting on `WithBot ℝ` preserving binary joins and `⊥`.
* `DirectedTransport.MaxAffineJoinAdapter.labelEquiv` and
  `DirectedTransport.MaxAffineJoinAdapter.labelMulEquiv`: the equivalence with max-affine
  labels of nonnegative slope, as an equivalence and as a monoid isomorphism.

## Main results

* `DirectedTransport.MaxAffineJoinAdapter.abstractApply_eq_applyBot`: the join-semidirect
