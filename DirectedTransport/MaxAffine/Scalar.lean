...
* `DirectedTransport.MaxAffineTransport.Label.residual`: the scalar residual
  `apply f x - x` of one label at a point.

## Main results

* `DirectedTransport.MaxAffineTransport.Label.apply_le_self_iff_of_slope_lt_one`,
  `DirectedTransport.MaxAffineTransport.Label.apply_le_self_iff_of_slope_eq_one`,
  `DirectedTransport.MaxAffineTransport.Label.apply_le_self_iff_of_one_lt_slope`: the
  pre-fixed set of one label in each slope regime.
* In the namespace `DirectedTransport.MaxAffineTransport.Label`, the `apply_eq_self_iff_*`
  and `not_exists_apply_eq_self_*` results: the complete description of the fixed set.
* `DirectedTransport.MaxAffineTransport.Label.exists_residual_le_of_slope_lt_one`,
  `DirectedTransport.MaxAffineTransport.Label.exists_residual_eq_shift_of_slope_eq_one`,
  `DirectedTransport.MaxAffineTransport.Label.residual_margin_of_one_lt_slope_of_floor_coe`:
  the attained/unbounded classification of the residual margin, stated without a
  real-valued `sInf`.
* `DirectedTransport.MaxAffineTransport.exists_edge_defect_ge_of_nonnegative_slopes`: the
  weighted edge obstruction at nonnegative slopes.
* `DirectedTransport.MaxAffineTransport.isFixedPt_foldl_rotate` and
  `DirectedTransport.MaxAffineTransport.isPrefixed_foldl_rotate`: forward rotation of exact
