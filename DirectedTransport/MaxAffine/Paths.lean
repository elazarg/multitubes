...
`DirectedTransport.MaxAffineTransport.Label.exact_defect_normal_form` combines these
branches into a nonlinear defect identity.

## Main definitions

* `DirectedTransport.MaxAffineTransport.Label.pathSlope`,
  `DirectedTransport.MaxAffineTransport.Label.pathShift`,
  `DirectedTransport.MaxAffineTransport.Label.pathFloor`: the three composite coefficients
  of a chronological list of labels, the floor valued in `WithBot ℝ`.
* `DirectedTransport.MaxAffineTransport.Label.weightedShiftTerms`,
  `DirectedTransport.MaxAffineTransport.Label.resetBranches`,
  `DirectedTransport.MaxAffineTransport.Label.resetBranchSup`: the explicit finite families
  behind those coefficients, one branch per last-reset position.
* `DirectedTransport.MaxAffineTransport.Label.recenter`,
  `DirectedTransport.MaxAffineTransport.Label.recenteredLabels`: the path acting on the
  initial discrepancy of a candidate chain.
* `DirectedTransport.MaxAffineTransport.Label.CandidateStep`,
  `DirectedTransport.MaxAffineTransport.Label.chainTarget`,
  `DirectedTransport.MaxAffineTransport.Label.chainLabels`: candidate chains.

## Main results

* `DirectedTransport.MaxAffineTransport.Label.compList_eq_pathCoefficients` and
  `DirectedTransport.MaxAffineTransport.Label.apply_compList_eq_pathCoefficients`: the
  composite label and its action are given by the path coefficients.
* `DirectedTransport.MaxAffineTransport.Label.pathFloor_eq_resetBranchSup` and
  `DirectedTransport.MaxAffineTransport.Label.pathFloor_le_iff_resetBranches`: the composite
  floor as the supremum of the last-reset branches.
* `DirectedTransport.MaxAffineTransport.Label.exact_defect_normal_form` and
  `DirectedTransport.MaxAffineTransport.Label.coe_exact_defect_eq_resetBranchSup_sup_affineSum`:
