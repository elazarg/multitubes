...
`DirectedTransport.MaxAffineTransport.eigenvalues_subset_relaxationLevels` places the eigenvalues
...
Splitting each label into its genuine branches -- an affine branch for every edge and a floor
branch for every finite floor -- turns the relaxed inequality `F x ≤ lam + x` into the finite
system `gamma_b + alpha_b * x (sigma_b) - x (tau_b) ≤ lam` of
`DirectedTransport.MaxAffine.Duality`, whose dual is the **normalized gain-flow polytope**: the
nonnegative branch weights of total mass one that balance, at every vertex, the mass arriving
against the gain-weighted mass leaving.  That polytope is a closed subset of the standard simplex,
hence compact, and threshold duality identifies the relaxation levels with the upper bounds of its
objective.  A least relaxation level is therefore the value of an optimal certificate.
...
(`DirectedTransport.MaxAffineTransport.eigenvalues_eq_univ_of_slope_eq`).

## Main definitions

* `DirectedTransport.MaxAffineTransport.branchEdge` and
  `DirectedTransport.MaxAffineTransport.branchSlope`: the edge and the gain of a genuine branch.
* `DirectedTransport.MaxAffineTransport.criticalVertices`: the targets of the branches a
  normalized certificate charges.

## Main results

* `DirectedTransport.MaxAffineTransport.mem_relaxationLevels_iff_forall_branchCertificateValue_le`:
  **the relaxation levels are the upper bounds of the normalized gain-flow objective.**
* `DirectedTransport.MaxAffineTransport.isCompact_setOf_isNormalizedBranchCertificate`: the
  gain-flow polytope is compact.
* `DirectedTransport.MaxAffineTransport.exists_isNormalizedBranchCertificate_value_eq`: a least
  relaxation level is the value of an optimal certificate.
* `DirectedTransport.MaxAffineTransport.branchResidual_eq_of_pos`: **complementary slackness**,
  every branch charged by an optimal certificate is tight.
* `DirectedTransport.MaxAffineTransport.source_mem_criticalVertices`: the critical set absorbs the
  positive-gain dependencies of the certificate.
* `DirectedTransport.MaxAffineTransport.vertexOperator_eq_of_mem_criticalVertices`: **the critical
  coordinates are frozen** by the relaxed operator.
* `DirectedTransport.MaxAffineTransport.isLeast_eigenvalues_of_isLeast_relaxationLevels`: **a least
  relaxation level is the least eigenvalue.**
* `DirectedTransport.MaxAffineTransport.exists_isLeast_eigenvalues_of_translation`: at floorless
  unit slopes the value is the maximum cycle mean.
* `DirectedTransport.MaxAffineTransport.not_isLeast_relaxationLevels_of_slope_eq`: at a common
  nonnegative slope other than one there is no least relaxation level.
* `DirectedTransport.MaxAffineTransport.isLeast_eigenvalues_loopLabel`: for the two-loop labelling
  the value is `5`.

## Implementation notes

The branch system is the one of `DirectedTransport.MaxAffine.Duality` rather than a fresh copy, so
that threshold strong duality applies verbatim.  What this file adds to it is the geometry of a
branch: `DirectedTransport.MaxAffineTransport.branchEdge` and
`DirectedTransport.MaxAffineTransport.branchSlope` name the edge and the gain that
`DirectedTransport.MaxAffineTransport.branchDelta` encodes implicitly, and
`DirectedTransport.MaxAffineTransport.branchBase_add_branchSlope_mul_le_apply` is the single fact
that a branch is below the label it comes from, which powers both the freezing step and the lower
bound along a walk.
...
`DirectedTransport.MaxAffineTransport.eigenvalues_subset_relaxationLevels` this identifies the
...
`DirectedTransport.MaxAffineTransport.exists_isEigenvector_of_translation`, so the formula agrees
...
`DirectedTransport.MaxAffineTransport.isLeast_eigenvalues_of_isLeast_relaxationLevels` is exactly
...
half-line above `5` (`DirectedTransport.MaxAffineTransport.relaxationLevels_loopLabel`), so the
formula returns `5`, in agreement with
`DirectedTransport.MaxAffineTransport.eigenvalues_loopLabel`.  The dual reading is the balance
