...
* `DirectedTransport.MaxAffineTransport.IsGenuineBranch` and
  `DirectedTransport.MaxAffineTransport.Branch`: the branch index, containing every affine
  branch and exactly the finite floor branches.
* `DirectedTransport.MaxAffineTransport.branchDelta`,
  `DirectedTransport.MaxAffineTransport.branchBase`,
  `DirectedTransport.MaxAffineTransport.branchResidual`: the branch inequality system and
  its residual.
* `DirectedTransport.MaxAffineTransport.IsNormalizedBranchCertificate` and
  `DirectedTransport.MaxAffineTransport.branchCertificateValue`: balanced mass-one dual
  certificates and their value.
* `DirectedTransport.MaxAffineTransport.WorstResidualAtMost`: attainability of a uniform
  residual threshold.

## Main results

* `DirectedTransport.MaxAffineTransport.isLaxSection_iff_forall_branch`: a lax section is
  exactly a solution of the branch system.
* `DirectedTransport.MaxAffineTransport.worstResidualAtMost_iff_exists_edge_defect_le`: the
  threshold restated edgewise.
* `DirectedTransport.MaxAffineTransport.branchCertificateValue_le_of_worstResidualAtMost`:
  weak duality.
* `DirectedTransport.MaxAffineTransport.worstResidualAtMost_iff_normalizedDual_le`:
  threshold strong duality, stated without assigning a real number to an infimum that may
  be `-∞`.
* `DirectedTransport.MaxAffineTransport.exists_isLaxSection_iff_normalizedBranchCertificate_nonpos`:
