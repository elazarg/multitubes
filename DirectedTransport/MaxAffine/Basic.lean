...
`DirectedTransport.MaxAffine.Additive` identifies unit-slope
...
`DirectedTransport.TransferSummary.reflectedIter`
(`holonomyApply_eq_reflectedIter`), whose survival-weighted representation is
`DirectedTransport.TransferSummary.reflectedIter_eq_sup'`.

`DirectedTransport.Basic` is the generic semantic layer.  `toTransport`
reads a max-affine labelling as a constant-fiber transport on `ℝ`, and
`holonomyApply` is `DirectedTransport.transport` at those edge actions.
The theorem `holonomyApply_eq_walkLabel_smul` identifies it with
`DirectedTransport.walkLabel` in the nonnegative-slope label monoid.
...
* `DirectedTransport.MaxAffineTransport.IsLaxSection`, `defect` -- the lax section and
  its edgewise residual.
* `DirectedTransport.MaxAffineTransport.holonomyApply` -- chronological transport along a
  walk.
* `DirectedTransport.MaxAffineTransport.slopeProd`, `suffixWeight`, `suffixWeightSum`,
  `weightedDefect` -- the survival-weighted accounting along an edge list.
* `DirectedTransport.MaxAffineTransport.cyclicLabel`, `cyclicChain`, `cyclicHolonomy` -- one
  turn of a cyclic max-affine system read as a composite label action.

## Main results

* `DirectedTransport.MaxAffineTransport.Label.apply_comp`,
  `DirectedTransport.MaxAffineTransport.Label.comp_assoc` -- the composition law, at
  nonnegative outer slope.
* `DirectedTransport.MaxAffineTransport.Label.apply_add_le` -- the one-sided Lipschitz
  estimate `apply f (x + d) ≤ apply f x + slope * d` for `0 ≤ d`.
* `DirectedTransport.MaxAffineTransport.holonomyApply_le_add_weightedDefect` -- the
...
* `DirectedTransport.MaxAffineTransport.holonomyApply_cycle_le` -- a lax section makes
  its value at the base vertex a pre-fixed point of every cycle's composite
  action.
* `DirectedTransport.MaxAffineTransport.Label.exists_apply_le_self_iff` -- the expansivity
...
* `DirectedTransport.MaxAffineTransport.exists_edge_defect_ge` -- the quantitative
  obstruction dual to the telescope.
* `DirectedTransport.MaxAffineTransport.cyclicSolution_cyclicChain` -- a fixed point of one
  turn of the cyclic labels is a `DirectedTransport.CyclicMaxAffine.CyclicSolution`.

## Implementation notes

This file develops the label algebra, walk semantics, weak duality, and the
quantitative obstruction.  `DirectedTransport.MaxAffine.Sections` proves section
...
`DirectedTransport.MaxAffineTransport.holonomyApply_eq_foldl_map`. -/
...
def holonomyApply (label : E → Label) {start finish : V}
  ...
  DirectedTransport.transport (fun e x => (label e).apply x) walk
...
holonomy `DirectedTransport.walkLabel` in the label monoid. -/
theorem holonomyApply_eq_walkLabel_smul
    (label : E → {f : Label // 0 ≤ f.slope}) {start finish : V}
    (walk : G.Walk start finish) (x : ℝ) :
    holonomyApply (fun e => (label e).1) walk x =
      DirectedTransport.walkLabel label walk • x :=
  DirectedTransport.transport_eq_smul (G := G) (label := label) walk x
...
the statement becomes that of `DirectedTransport.MaxPlusPotential.exists_edge_defect_ge`. -/
...
`DirectedTransport` develops transport with vertex-indexed fibers; a
