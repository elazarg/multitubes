...
* `DirectedTransport.EdgeGraph`: a directed multigraph as vertex and edge types with `source`
  and `target` maps.
* `DirectedTransport.EdgeGraph.Walk`: an endpoint-indexed finite walk.
* `DirectedTransport.EdgeGraph.Walk.edges`, `length`, `visited`, `edgeMultiplicity`: the
  basic measurements of a walk.
* `DirectedTransport.EdgeGraph.Walk.append`, `singleton`, `castFinish`: walk constructors.
* `DirectedTransport.EdgeGraph.Walk.VertexSplit`: a witness that a walk visits a vertex,
...
* `DirectedTransport.LinkedTo`, `DirectedTransport.IsStronglyConnectedAt`, and
  `DirectedTransport.EdgeEndpointsLinkedTo`: the walk-existence reachability scopes
  consumed by the potential and normal-form theory.

## Main results

* `DirectedTransport.EdgeGraph.Walk.edges_isChain`: consecutive edges have matching endpoints.
...
* `DirectedTransport.EdgeGraph.Walk.VertexSplit.length_splice`: splicing adds lengths.
