...
retract theories: one-base flatness of a transport (`DirectedTransport.Exact`)
...
one, so the path-functor translation of `DirectedTransport.Category` carries an
actual theorem rather than only data.

## Main definitions

* `DirectedTransport.categoricalRetract`: the `CategoryTheory.Retract` exhibiting
  the base object as a retract of each vertex object, built from the chosen ingress and return
  morphisms.

## Main results

* `DirectedTransport.categoricalRetract_i`, `DirectedTransport.categoricalRetract_r`, and
  `DirectedTransport.categoricalRetract_r_comp_i`: the retract's data unfolds to the
  ingress and return morphisms.
* `DirectedTransport.categorical_compressed_map_eq_retract_r_comp_i`: the compressed core
  morphism is the retract's return followed by the retract's ingress.
* `DirectedTransport.Transport.isFlatAt_toPathFunctor_iff`: one-base flatness of a
  transport is flatness of its path functor.
* `DirectedTransport.Transport.compressed_walkMap_eq_via_category`: the concrete retract
  normal form of `DirectedTransport.Exact`, rederived through the path-functor dictionary.
-/
...
`DirectedTransport.Exact` proves the retract normal form concretely, by walk
induction, and `DirectedTransport.CategoricalRetracts` proves it categorically.
The two developments meet along `DirectedTransport.Transport.toPathFunctor`:
...
`DirectedTransport.IsFlatAt` transfers across the path-functor dictionary. -/
...
/-- `DirectedTransport.Transport.compressed_walkMap_eq`, rederived from the
