...
* `DirectedTransport.finiteInequalityMatrix`: the row data transported along the canonical
...
* `DirectedTransport.not_nonnegative_incompatibility_of_potential`: the two alternatives
  exclude each other.
* `DirectedTransport.exists_potential_iff_no_nonnegative_incompatibility`: the two combined as
  an equivalence.  This is the form other files use.
* `DirectedTransport.exists_potential_or_bellmanResidual_obstruction`: under the residual
