...
* `DirectedTransport.Transport.pathClosure`: the join over all directed walks of the
  transported values of a given lower bound.
* `DirectedTransport.Transport.PathClosureSupSpec`: the hypothesis under which that join is
  computed edgewise.
* `DirectedTransport.Transport.rootedLower`: the lower bound concentrated at one root.
* `DirectedTransport.Transport.bellman` and `DirectedTransport.Transport.bellmanOrderHom`:
  the Bellman operator of a monotone transport, as a function and as an order homomorphism.
* `DirectedTransport.Transport.leastLaxMajorant`: its least fixed point above a given lower
  bound.

## Main results

* `DirectedTransport.Transport.isLaxSection_pathClosure` and
  `DirectedTransport.Transport.pathClosure_isLeast`: the path closure is the least lax
  section above the given lower bound.
* `DirectedTransport.Transport.pathClosure_isLeast_of_sSupHom`: the same conclusion for
  arbitrary-join-preserving edge maps.
* `DirectedTransport.Transport.rooted_pathClosure_isLeast`: the rooted form, where one
  pre-fixed witness at the root suffices.
* `DirectedTransport.Transport.bellman_le_iff`: lax sections are exactly the pre-fixed
  points of the Bellman operator.
* `DirectedTransport.Transport.leastLaxMajorant_isLeast`: monotone edge maps admit a least
  lax majorant.
...
`DirectedTransport.MaxPlusPotential.maxIncomingWeight` and
`DirectedTransport.ChargedPathBudget.ChargedRelation.value` are the
conditionally complete counterparts of `DirectedTransport.Transport.pathClosure`
