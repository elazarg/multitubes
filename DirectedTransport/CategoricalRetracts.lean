...
* `DirectedTransport.IsFlatAt`: a path functor sends every endomorphism at one object to
  the identity.
* `DirectedTransport.categoricalIngress` and `DirectedTransport.categoricalReturn`: the
  morphisms to and from the base object determined by chosen path families.
* `DirectedTransport.categoricalRetractProjector`: the resulting endomorphism at each
  vertex object.

## Main results

* `DirectedTransport.categoricalIngress_comp_return`: ingress followed by return is the
  identity, so the projectors split.
* `DirectedTransport.categoricalRetractProjector_idempotent`: the projectors are
  idempotent.
* `DirectedTransport.categorical_compressed_map_eq`: compressing a path by its endpoint
  projectors gives a path-independent core morphism.
* `DirectedTransport.categorical_coreMap_comp_reverse` and
  `DirectedTransport.categorical_reverse_comp_coreMap`: the core morphisms are mutually
