...
* `DirectedTransport.ChargedPathBudget.ChargedRelation`: edges with source, target, and
  nonnegative charge.
* `DirectedTransport.ChargedPathBudget.ChargedRelation.Path`: finite admissible paths, indexed
  by their endpoints.
* `DirectedTransport.ChargedPathBudget.ChargedRelation.Path.highChargeCount`: number of edges
  above a charge threshold.
* `DirectedTransport.ChargedPathBudget.ChargedRelation.budget` and
  `DirectedTransport.ChargedPathBudget.ChargedRelation.HasFiniteBudget`: the path budget.
* `DirectedTransport.ChargedPathBudget.ChargedRelation.budgetEN`: the same budget as a totally
  defined `ℝ≥0∞`.
* `DirectedTransport.ChargedPathBudget.ChargedRelation.value`: the budget-to-go function.
* `DirectedTransport.ChargedPathBudget.ChargedRelation.IsPotential` and
  `DirectedTransport.ChargedPathBudget.ChargedRelation.IsBoundedPotential`.

## Main results

* `DirectedTransport.ChargedPathBudget.ChargedRelation.hasFiniteBudget_iff_exists_boundedPotential`:
...
* `DirectedTransport.ChargedPathBudget.ChargedRelation.budget_eq_oscillation_value` and
  `DirectedTransport.ChargedPathBudget.ChargedRelation.isLeast_oscillation`: strong duality, the
  budget is the *minimum* oscillation of a bounded potential, and the minimum is attained by
  `value`. The two inequalities
  `DirectedTransport.ChargedPathBudget.ChargedRelation.budget_le_oscillation` and
  `DirectedTransport.ChargedPathBudget.ChargedRelation.oscillation_value_le_budget` are stated
  separately and are usable on their own.
* `DirectedTransport.ChargedPathBudget.ChargedRelation.budgetEN_eq_iInf_oscillation`: the same
...
* `DirectedTransport.ChargedPathBudget.ChargedRelation.isLeast_value` and
  `DirectedTransport.ChargedPathBudget.ChargedRelation.value_le_iff`: Bellman characterisation,
  `value` is the least nonnegative supersolution.
* In the namespace `DirectedTransport.ChargedPathBudget.ChargedRelation`, the results
