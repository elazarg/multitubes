...
(`DirectedTransport.TransferSummary.bandIter_eq_floorIter`).
...
* A **clamped affine summary** `(lo, hi, shift, slope)` acts by
  ...
  outer map (`DirectedTransport.TransferSummary.ClampedAffineSummary.apply_comp`): a word of
  arbitrary length in these maps still has four coefficients
  (`DirectedTransport.TransferSummary.ClampedAffineSummary.apply_foldr`). The two-sided class
  needs one coefficient more than the max-affine class and, like it, has no identity.

* The **two-sided orbit** `DirectedTransport.TransferSummary.bandIter` iterates the clamped
...
* A **Lindley-style representation** survives, in an inf-sup form. The one-sided state is a
  ...
  suprema (`DirectedTransport.TransferSummary.bandIter_eq_inf'_sup'`), of which the term that
...
* The **backward construction** in the band converges with no drift condition at all
  (`DirectedTransport.TransferSummary.tendsto_pastBandIter_hi`,
  `DirectedTransport.TransferSummary.tendsto_pastBandIter_lo`): the band itself supplies the
  ...
  (`DirectedTransport.TransferSummary.tendsto_pastBandIter_hi_ciSup`), and there it needs no
  hypothesis on the service.

## Main definitions

* `DirectedTransport.TransferSummary.ClampedAffineSummary` and
  `DirectedTransport.TransferSummary.ClampedAffineSummary.comp`.
* `DirectedTransport.TransferSummary.floorIter` -- the one-sided orbit above a general floor.
* `DirectedTransport.TransferSummary.bandIter` -- the two-sided orbit.
* `DirectedTransport.TransferSummary.bandRestart` -- the one-sided orbit that restarts from the
  top of the band.
* `DirectedTransport.TransferSummary.pastBandIter` -- the backward two-sided orbit.

## Main results

* `DirectedTransport.TransferSummary.ClampedAffineSummary.apply_comp`,
  `DirectedTransport.TransferSummary.ClampedAffineSummary.comp_assoc` and
  `DirectedTransport.TransferSummary.ClampedAffineSummary.apply_foldr` -- clamped affine words
  keep four coefficients.
* `DirectedTransport.TransferSummary.ClampedAffineSummary.apply_mem_Icc` and
  `DirectedTransport.TransferSummary.ClampedAffineSummary.not_exists_apply_eq_id`.
* `DirectedTransport.TransferSummary.bandIter_mem_Icc`,
  `DirectedTransport.TransferSummary.monotone_bandIter` and
  `DirectedTransport.TransferSummary.bandIter_le_of_le_service` -- confinement to the band,
  monotone in the initial state, antitone in the service sequence.
* `DirectedTransport.TransferSummary.bandIter_eq_floorIter` -- the two-sided orbit is the
  one-sided one wherever the upper clamp does not bind.
* `DirectedTransport.TransferSummary.floorIter_eq_sup'` -- Lindley's representation above a
  general floor.
* `DirectedTransport.TransferSummary.bandIter_eq_inf'` and
  `DirectedTransport.TransferSummary.bandIter_eq_inf'_sup'` -- the two-sided representation, as
  an infimum over upper restarts and in closed form.
* `DirectedTransport.TransferSummary.bandIter_reversed` -- the backward orbit is the forward
  orbit of the input read forwards.
* `DirectedTransport.TransferSummary.tendsto_pastBandIter_hi` and
  `DirectedTransport.TransferSummary.tendsto_pastBandIter_lo` -- convergence of the backward
  construction, with no condition on the input.
* `DirectedTransport.TransferSummary.pastBandIter_sub_le` and
  `DirectedTransport.TransferSummary.tendsto_pastBandIter_hi_ciSup` -- under geometric retention
  the two extreme backward orbits meet, and by
  `DirectedTransport.TransferSummary.tendsto_pastBandIter` the limit is then reached from every
  state of the band.
...
Lindley's representation `DirectedTransport.TransferSummary.reflectedIter_eq_sup'`; the terms
...
`DirectedTransport.TransferSummary.reflectedIter_reversed`. -/
...
two-sided analogue of `DirectedTransport.TransferSummary.monotone_loynes`. -/
...
construction needs for `DirectedTransport.TransferSummary.tendsto_loynes_ciSup`. -/
