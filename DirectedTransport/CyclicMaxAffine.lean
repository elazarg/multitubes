...
The consequence worth having is `DirectedTransport.CyclicMaxAffine.CyclicSolution.le_max_of_sum_le`:
if `∑_{k < L} p k ≤ A` then
...
* `DirectedTransport.CyclicMaxAffine.survivalProduct`: the product of the survival factors
  strictly before a phase.
* `DirectedTransport.CyclicMaxAffine.weightedRate`: the survival-weighted accumulated rate.
* `DirectedTransport.CyclicMaxAffine.CyclicSolution`: a solution of the cyclic max-affine system,
  unrolled as a linear chain closed by a wrap condition.
* `DirectedTransport.CyclicMaxAffine.finWeightedRate`: the weighted rate of a `Fin L`-indexed
  cycle.

## Main results

* `DirectedTransport.CyclicMaxAffine.CyclicSolution.weightedRate_div_le`: the lower bound.
* `DirectedTransport.CyclicMaxAffine.CyclicSolution.le_max_weightedRate`: the upper bound.
* `DirectedTransport.CyclicMaxAffine.CyclicSolution.weightedRate_bounds`: both bounds at once.
* `DirectedTransport.CyclicMaxAffine.CyclicSolution.le_max_of_sum_le`: the `L`-free corollary.
* `DirectedTransport.CyclicMaxAffine.CyclicSolution.unique_at_zero`: the solution is unique at
  the base point.
* `DirectedTransport.CyclicMaxAffine.fin_weightedRate_bounds`,
  `DirectedTransport.CyclicMaxAffine.fin_le_max_of_sum_le`: both in genuinely cyclic `Fin L` form.
...
namespace DirectedTransport.CyclicMaxAffine
...
end DirectedTransport.CyclicMaxAffine
