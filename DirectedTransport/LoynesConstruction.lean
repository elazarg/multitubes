...
Lindley's representation `DirectedTransport.TransferSummary.reflectedIter_eq_sup'` writes the
...
* `DirectedTransport.TransferSummary.pastTransport` and
  `DirectedTransport.TransferSummary.pastService` -- the retention and the retention-weighted
  service accumulated over the last `j` stages, indexed by age.
* `DirectedTransport.TransferSummary.loynes` -- the finite-horizon Loynes value.
* `DirectedTransport.TransferSummary.reversed` -- an age-indexed input read forwards over a
  fixed horizon.

## Main results

* `DirectedTransport.TransferSummary.reflectedIter_reversed` -- the finite-horizon Loynes value
  is the reflected orbit of the reversed input started from `0`.
* `DirectedTransport.TransferSummary.monotone_loynes` and
  `DirectedTransport.TransferSummary.loynes_nonneg`.
* `DirectedTransport.TransferSummary.loynes_le` -- geometric retention and a lower bound on the
  service bound the whole construction by `C / (1 - ρ)`.
* `DirectedTransport.TransferSummary.tendsto_loynes_ciSup` -- monotone convergence of the
