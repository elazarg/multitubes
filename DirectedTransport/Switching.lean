...
switch: `DirectedTransport.switch` takes `η : V → Mˣ`.  Over a group every element is a unit
...
Together with `DirectedTransport.switch_one` this makes switching an action of the group
