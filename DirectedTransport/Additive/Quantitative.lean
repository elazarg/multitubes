...
* `DirectedTransport.AdditiveTransport.WorstDirectedResidualAtMost`: existence of a potential
  whose largest directed additive residual is at most a given level.
* `DirectedTransport.AdditiveTransport.SymmetricResidualAtMost`: the same with both signed
  residual rows bounded, so the defect is bounded in absolute value.
* `DirectedTransport.AdditiveTransport.SignedEdge`, `signedDelta` and `signedBase`: the two
  orientations of each edge, the incidence row in the selected orientation, and the weight
  carrying the orientation's sign.

## Main results

* `DirectedTransport.AdditiveTransport.worstDirectedResidualAtMost_iff_closedWalk_le`: a
...
* `DirectedTransport.AdditiveTransport.symmetricResidualAtMost_iff_finiteInequality`: symmetric
  signed-row feasibility is exactly the doubled finite row system, the bridge to
  `DirectedTransport.FiniteInequality`.
* `DirectedTransport.AdditiveTransport.symmetricResidualAtMost_iff_normalizedSignedDual_le`: the
  exact symmetric dual, characterizing the threshold by balanced probability distributions on
  signed edge rows.
* `DirectedTransport.AdditiveTransport.exists_zero_defect_iff_normalizedSignedDual_nonpos`: at
