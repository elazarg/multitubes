...
* `DirectedTransport.AdditiveTransport.IsImmediateSignedBacktrack` and
...
* `DirectedTransport.AdditiveTransport.signedCirculationObjective` and
...
* `DirectedTransport.AdditiveTransport.normalizedCertificate_mem_convexHull_simpleSignedCircuits`
...
* `DirectedTransport.AdditiveTransport.isUnitSignedCirculation_iff_exists_normalizedLift`: the
...
* `DirectedTransport.AdditiveTransport.exists_absoluteSignedCircuitOptimum`: that support
  optimum is attained.
