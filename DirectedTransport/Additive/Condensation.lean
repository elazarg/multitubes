...
`DirectedTransport.MaxPlusPotential.exists_isPotential_iff_forall_closedWalk_nonpos` is
...
`DirectedTransport.sccCondensation` is performed: the componentwise potentials certify
...
`DirectedTransport.hasTrivialCycleLabels_iff_hasSCCUnitPotentials`, and a preprocessing
statement beside the simple-cycle reduction
`DirectedTransport.AdditiveTransport.worstDirectedResidualAtMost_iff_simpleCycles_le`:
...
* `DirectedTransport.EdgeGraph.induce`: the induced subgraph on a set of vertices; an
  edge belongs to it exactly when both of its endpoints do.
* `DirectedTransport.sccSet`: the vertex set of a strongly connected component, as the
  fiber of `DirectedTransport.toSCC` over it.
* `DirectedTransport.MaxPlusPotential.HasSCCPotentialAt` and
  `DirectedTransport.MaxPlusPotential.HasSCCPotentials`: a potential on one strongly
  connected component, respectively on every component.

## Main results

* `DirectedTransport.MaxPlusPotential.exists_isPotential_iff_forall_scc_induce`: a
  potential exists exactly when the induced subgraph of every strongly connected
  component admits one.
* `DirectedTransport.MaxPlusPotential.exists_isPotential_iff_hasSCCPotentials`: the
  same decomposition phrased through componentwise constrained potentials.
* `DirectedTransport.MaxPlusPotential.hasSCCPotentialAt_iff_exists_isPotential_induce`:
  the two componentwise phrasings agree, without any finiteness hypothesis.
* `DirectedTransport.source_mem_sccSet_of_mem_edges` and
  `DirectedTransport.target_mem_sccSet_of_mem_edges`: a closed walk stays inside the
...
`DirectedTransport.toSCC` over it. -/
