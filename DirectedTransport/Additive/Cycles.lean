...
* `DirectedTransport.AdditiveTransport.IsSimpleCycle`: a nonempty closed walk with no
...
* `DirectedTransport.AdditiveTransport.CycleMeanDominates`: one closed walk has mean weight at
  least that of another, stated by cross-multiplication so that it is meaningful without
  dividing by a length.

## Main results

* `DirectedTransport.AdditiveTransport.exists_simpleCycle_cycleMeanDominates`: every nonempty
...
* `DirectedTransport.AdditiveTransport.forall_closedWalk_mean_le_iff_simpleCycle`: a cycle-mean
  bound holds for all closed walks exactly when it holds for the simple ones.
* `DirectedTransport.AdditiveTransport.worstDirectedResidualAtMost_iff_simpleCycles_le`: the
