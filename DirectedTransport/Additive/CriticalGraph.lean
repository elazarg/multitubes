...
`DirectedTransport.Additive.Eigenvector` produces one max-plus eigenvector for the maximum cycle
mean `lam`: the column of the Kleene star of `weight - lam` rooted at a critical vertex.  This
...
* `DirectedTransport.EdgeGraph.restrictEdges`: the subgraph on a set of edges, the edge-indexed
  analogue of `DirectedTransport.EdgeGraph.induce`.
* `DirectedTransport.EdgeGraph.Walk.ofRestrictEdges`: a walk of an edge subgraph read as a walk
  of the ambient graph.
* `DirectedTransport.MaxPlusPotential.IsCriticalEdge`,
  `DirectedTransport.MaxPlusPotential.criticalEdges`: an edge lying on a closed walk of mean
  weight `lam`, and the set of these.
* `DirectedTransport.MaxPlusPotential.criticalGraph`,
  `DirectedTransport.MaxPlusPotential.criticalWeight`: the critical subgraph and the restriction
  of the weighting to it, with `DirectedTransport.MaxPlusPotential.liftCriticalWalk` reading a
...
* `DirectedTransport.MaxPlusPotential.defect_eq_zero_of_isCriticalEdge`: a critical edge is tight
  for *every* potential of the shifted weighting.
* `DirectedTransport.MaxPlusPotential.walkWeight_criticalGraph_eq`: **the characteristic property
  of the critical graph**, every closed walk of it has mean exactly `lam`.
* `DirectedTransport.MaxPlusPotential.isCriticalVertex_of_criticalGraph_closedWalk`: consequently
  every vertex of a nonempty closed walk of the critical graph is a critical vertex.
* `DirectedTransport.MaxPlusPotential.add_maxRootedWeight_le_maxRootedWeight`: the triangle
  inequality of the Kleene star, and
  `DirectedTransport.MaxPlusPotential.isGreatest_range_maxRootedWeight`: the canonical potential
  `DirectedTransport.MaxPlusPotential.maxIncomingWeight` is the largest Kleene-star entry in its
  column, so it is one row of the all-pairs operator.
* `DirectedTransport.MaxPlusPotential.isGraphEigenvector_maxRootedWeight`: **every critical
  Kleene-star column is an eigenvector.**
* `DirectedTransport.MaxPlusPotential.isGreatest_criticalColumns`: **every eigenvector is the
...
`DirectedTransport.EdgeGraph.restrictEdges` is placed here, beside its only use, exactly as
`DirectedTransport.EdgeGraph.induce` is placed beside its own in
`DirectedTransport.Additive.Condensation`; both are one-line reindexings of an ambient graph
...
`DirectedTransport.EdgeGraph.induce`, which selects a set of vertices instead. -/
...
`DirectedTransport.MaxPlusPotential.maxRootedWeight` is the all-pairs longest-walk operator, the
...
(`DirectedTransport.MaxPlusPotential.maxRootedWeight_self`), composition is subadditive, and
`DirectedTransport.MaxPlusPotential.maxIncomingWeight` is the largest entry of a column. -/
...
`DirectedTransport.MaxPlusPotential.IsEigenvector`, by
`DirectedTransport.MaxPlusPotential.isEigenvector_iff_isGraphEigenvector`. -/
