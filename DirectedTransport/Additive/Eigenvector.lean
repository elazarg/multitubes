...
here, as `DirectedTransport.MaxPlusPotential.IsCriticalVertex`.  The subgraph itself, and the
description of the whole eigenspace as generated by the Kleene-star columns rooted at critical
vertices, are `DirectedTransport.Additive.CriticalGraph`.  So are the critical *classes* - the
...
class - together with the open question of that family's minimality.
