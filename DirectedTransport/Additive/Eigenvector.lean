...
`DirectedTransport.Additive.Potentials` produces *sub*eigenvectors: functions with
...
* `DirectedTransport.MaxPlusPotential.rootedWeights`,
  `DirectedTransport.MaxPlusPotential.nodupRootedWeights`: the weights of the walks issuing from
  a fixed base vertex and arriving at a given vertex, and of those among them that repeat no
  edge.
* `DirectedTransport.MaxPlusPotential.maxRootedWeight`: the greatest such weight, the rooted
  analogue of `DirectedTransport.MaxPlusPotential.maxIncomingWeight` and the row of the Kleene
  star indexed by the base.
* `DirectedTransport.MaxPlusPotential.IsCriticalVertex`: a vertex carrying a closed walk of mean
  weight exactly `lam`.
* `DirectedTransport.MaxPlusPotential.IsEigenvector`: the max-plus eigenvalue equation
  `A ⊗ v = lam ⊗ v`.

## Main results

* `DirectedTransport.MaxPlusPotential.maxRootedWeight_isPotential`: the rooted maximum is a
  potential, that is, a subeigenvector.
* `DirectedTransport.MaxPlusPotential.exists_tight_edge_maxRootedWeight`: **tightness**, the new
...
* `DirectedTransport.MaxPlusPotential.isEigenvector_iff`: the eigenvalue equation splits into the
  subeigenvector inequality and tightness at every coordinate.
* `DirectedTransport.MaxPlusPotential.exists_isEigenvector_of_maximizing_cycle`: **existence of a
...
* `DirectedTransport.MaxPlusPotential.exists_isEigenvector`: over the reals the maximizing cycle
...
The graph-level results are stated for an arbitrary `DirectedTransport.EdgeGraph` and ask
explicitly for the reachability they use: `∀ vertex, Nonempty (G.Walk base vertex)`, one half of
`DirectedTransport.IsStronglyConnectedAt`.  Only walks *out of* the base occur, so the
return direction is never needed.  For the matrix reading no reachability hypothesis survives:
`DirectedTransport.MaxPlusPotential.matrixGraph` is the complete digraph, since a matrix with
...
`DirectedTransport.MaxPlusPotential.maxRootedWeight` is total, taking the junk value `0` when no
walk from the base arrives, in the manner of
`DirectedTransport.MaxPlusPotential.stepValue`; every result about it assumes reachability, under
which that case never occurs.

Following `DirectedTransport.Additive.CycleMean`, the general-field theorem takes the
mean-maximizing cycle as a hypothesis and only the real version invokes attainment, which is
available over `ℝ` alone.

The critical *graph* is not developed: since the eigenvalue equation is proved at every
coordinate, no such restriction is needed for existence.  Only the critical *vertex* used as the
root appears, as `DirectedTransport.MaxPlusPotential.IsCriticalVertex`.

## TODO

The critical graph itself — the subgraph of edges lying on closed walks of mean `lam` — and the
description of the whole eigenspace as generated by the Kleene-star columns rooted at the
critical classes.
...
`DirectedTransport.MaxPlusPotential.maxIncomingWeight_isPotential`, and needs the source of every
...
coordinatewise.  This is `DirectedTransport.MaxPlusPotential.IsSubeigenvector` with the
