...
* `DirectedTransport.AdditiveTransport.signedGraph`: the doubled graph, whose edges are the two
  orientations of each original edge.
* `DirectedTransport.AdditiveTransport.circuitCoefficient`: the normalized edge-multiplicity
...
* `DirectedTransport.AdditiveTransport.symmetricResidualAtMost_iff_signedClosedWalk_le` and
