...
`DirectedTransport`; this file is its additive translation
...
`DirectedTransport` owns typed walk composition, constant-fiber actions,
...
`DirectedTransport.Additive.Potentials` develops the one-sided inequality
`φ (source e) + w e ≤ φ (target e)` and its positive-cycle duality.  Exactness
is its zero-defect case.  `DirectedTransport.ChargedRelation` uses the opposite decrement
...
`DirectedTransport.Basic`, and the reachability scopes
`DirectedTransport.LinkedTo`, `DirectedTransport.IsStronglyConnectedAt`, and
`DirectedTransport.EdgeEndpointsLinkedTo` live in
`DirectedTransport.EdgeGraph`; both are generic walk calculus.
...
The generic fold `DirectedTransport.walkSum` and its `simp` calculus live with
the monoid walk label in `DirectedTransport.Basic`; this section relates it to
the real walk weight and records its subtraction rule. -/

/-- The real walk weight of `DirectedTransport.Additive.Potentials` is the real instance of
`walkSum`. -/
theorem walkWeight_eq_walkSum (weight : E → ℝ) {start finish : V}
    (walk : G.Walk start finish) :
    DirectedTransport.MaxPlusPotential.walkWeight weight walk = walkSum weight walk := rfl
...
The residual of a candidate potential at one edge is `DirectedTransport.MaxPlusPotential.defect`,
and the one-sided existence statement is `DirectedTransport.MaxPlusPotential.exists_edge_defect_ge`.
Exactness is the case where that residual vanishes on every edge. -/
section Quantitative

open DirectedTransport.MaxPlusPotential
...
`DirectedTransport.MaxPlusPotential.exists_edge_defect_ge` with the length hypothesis
...
def translationTransport (G : EdgeGraph V E) (w : E → A) :
    DirectedTransport.Transport G (fun _ : V => A) :=
  DirectedTransport.ofEdgeAct G A fun edge point => point + w edge
...
@[simp] theorem walkMap_translationTransport (w : E → A)
  ...
  | nil => simp
  | concat walkSoFar edge legal ih =>
      rw [DirectedTransport.Transport.walkMap_concat,
        DirectedTransport.fiberCast_const, ih, walkSum_concat]
...
`DirectedTransport.ChargedRelation` characterises finiteness of the path budget of
...
open DirectedTransport.ChargedPathBudget
...
`DirectedTransport.MaxPlusPotential.IsPotential`, hence the negation. -/
